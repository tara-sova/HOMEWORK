...
let dFs_y k x y = (sh(k * (x ** 2.0) + y) + 1.0) * y / y * k / k
...
let rec newton (x_0 : float) (y_0 : float) (f : float -> float -> float) (g : float -> float -> float) (dF_x : float -> float -> float) (dF_y : float -> float -> float) (dG_x : float -> float -> float) (dG_y : float -> float -> float) (step : int) = 
   // let a = System.Math.Round(x_0, 5)
    System.Console.WriteLine("{0}    {1}    {2}    {3}    {4}"
                             , step, System.Math.Round(x_0, 6), System.Math.Round(y_0, 6), f x_0 y_0, g x_0 y_0)
    ...
    if abs(x_k - x_0) <= eps && abs(y_k - y_0) <= eps then
       System.Console.WriteLine("{0}    {1}    {2}    {3}    {4}", step + 1, x_k, y_k, f x_k y_k, g x_k y_k)
    else
