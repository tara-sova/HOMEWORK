...
void print(List *list)
{
	...
	cout << "Number of the capital of this country: " << list->vertexNumber << ' ' << endl;

	cout << "Cities in this country: ";
...
void addVertexToList(List* list[], int counter, int k, int minvertex)
{
	for (int i = 0; i < k; ++i)
	{
		if (list[i]->vertexNumber == minvertex)
		{
			insertToHead(list[i], counter);
			return;
		}
	}
}