...
let rec solve (f : float -> float) (dF : float -> float) (x_0 : float) counter = 
    ...
    if abs difference > eps then 
       System.Console.WriteLine("{0}    {1},    {2}", int32(counter), System.Math.Round(x_0, 4), System.Math.Round(px, 4))
...
let mainFunc = 
    System.Console.WriteLine("function = 128.0 * (x ^ 3) - 256.0 * (x ^ 6) + 160.0 * (x ^ 4) - 32.0 * (x ^ 2) + 1.0
                              \ninterval = (0.058, 2.122)")
    act (0.058, 2.122) p1 dP1
    System.Console.WriteLine("\ninterval = (-0.47, -0.058)")
    act (-0.47, -0.058) p1 dP1

    System.Console.WriteLine("function = 3.0 * (x ^ 2) + 4.0 * x - 1.75
                              \ninterval = (0.44, 2.3)")
    act (0.44, 2.3) p2 dP2
    System.Console.WriteLine("\ninterval = (-3.0, -0.38)")
    act (-3.0, -0.38) p2 dP2
